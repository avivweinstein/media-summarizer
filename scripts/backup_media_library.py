...
def create_backup(
    ...
    *,
    ...
    try:
        ...
        shutil.copytree(vault, temporary / "vault", symlinks=True)
        manifest = {
...
def restore_backup(snapshot: Path, database_destination: Path, vault_destination: Path) -> None:
    ...
    shutil.copy2(snapshot / "jobs.db", database_destination)
    database_destination.chmod(0o600)
    shutil.copytree(snapshot / "vault", vault_destination, symlinks=True)
